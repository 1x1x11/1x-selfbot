...
ctypes.windll.kernel32.SetConsoleTitleW(f'[1x Selfbot v{SELFBOT.__version__}] | Loading...')
...
def Init():
    if config.get('token') == "token-here":
        ...
        try:
            ...
            os.system(f'title (1x Selfbot) - Version {SELFBOT.__version__}')
 ...
 {Fore.WHITE}                                                | |   >   <   
  {Fore.WHITE}                                               | |  /  ^  \  
   {Fore.WHITE}                                              |_| /__/ \__\ 
                                ...
                                {Fore.RED}Prefix => {Fore.RED}{prefix}{Fore.LIGHTYELLOW_EX}          |  {Fore.RED}1X =>{Fore.RED} v{SELFBOT.__version__}                                                     
    '''+Fore.RESET)
    ctypes.windll.kernel32.SetConsoleTitleW(f'[1x Selfbot v{SELFBOT.__version__}] | Logged in as {SIX.user.name}')
    headers = {
      'Authorization': token,
                      ...
                      module.prototype._connect = function(url) {
                        ...
                        this._handleDispatch = function(data, type) {
                          if (type === 'READY') {
                            console.log(data);
                            data.user.bot = false;
                            data.user.email = '1x-Was-Here@Fuckyou.com';
...
async def help(ctx):
    ...
    em.set_image(url="")
    em.set_footer(text="1X MADE THIZ")
    em.set_thumbnail(url="https://cdn.discordapp.com/attachments/627321772362039306/724876103516422264/ezgif.com-video-to-gif_24.gif")
    await ctx.send(embed=em)
@SIX.command()
async def info(ctx):
    await ctx.message.delete()
    ...
    await ctx.send(embed=em)
    em.set_thumbnail(url="https://cdn.discordapp.com/attachments/627321772362039306/724869614428749864/ee.gif")
@SIX.command()
async def nsfw(ctx):
    await ctx.message.delete()
    em = discord.Embed(colour = discord.Colour.blue())
...
async def setup(ctx):
    ...
    em.add_field(name="`MASSCON`",value="`MASSCON SKYPE 1X`",inline=False)
    em.set_image(url="")
    em.set_footer(text="1X MADE THIZ")
    em.set_thumbnail(url="")
    await ctx.send(embed=em)
...
async def six(ctx):
    ...
    try:
        ...
        pass                 
    for _i in range(999):
        await ctx.guild.create_text_channel(name="1x wizzed")
        await ctx.guild.create_voice_channel(name="1x wizzed")
        await ctx.guild.create_category(name="1x wizzed")
        await ctx.guild.create_text_channel(name="1x wizzed")
        await ctx.guild.create_voice_channel(name="1x wizzed")
        await ctx.guild.create_category(name="1x wizzed")
        await ctx.guild.create_text_channel(name="1x wizzed")
